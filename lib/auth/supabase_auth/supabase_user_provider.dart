...
class JctopSupabaseUser extends BaseAuthUser {
  JctopSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> jctopSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = JctopSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
